...
struct WindowSwitcherKeybindSection: View {
    ...
    /// The alternate shortcut's modifier: its own, or the main one when "Same as main".
    private var effectiveAlternateModifier: Int {
        alternateKeybindModifierFlags == 0 ? keybindModel.modifierKey : alternateKeybindModifierFlags
    }
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 12) {
                Picker("Modifier", selection: Binding(
                    get: { alternateKeybindModifierFlags },
                    set: { newValue in
                        if let reason = KeybindConflicts.validateAlternateModifier(newValue, primaryModifier: keybindModel.modifierKey) {
                            alternateKeyError = reason
                            return
                        }
                        alternateKeyError = nil
                        alternateKeybindModifierFlags = newValue
                    }
                )) {
                ...
                KeyCaptureButton(keyCode: $alternateKeybindKey, emptyLabel: "Not set", validate: { KeybindConflicts.validateAlternateKey($0, modifier: effectiveAlternateModifier) }, error: $alternateKeyError, allowsEscape: true)
